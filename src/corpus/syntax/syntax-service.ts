      ...
      try {
        ...
        return new SyntaxGraph(graph, edgeLabels, segmentNodeCount);
          ...
          if (error instanceof Error) { // Checks if error is an instance of the Error class
              console.error('Error fetching syntax data:', error.message);
              throw new Error('Failed to load syntax data. Error: ' + error.message);
          } else {
              // Handle cases where the caught value isn't an Error object
              console.error('An unexpected error occurred with syntax data:', error);
              throw new Error('An unexpected error occurred with syntax data');
          }
      }
    }
